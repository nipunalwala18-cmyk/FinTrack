...
export const sendVerificationEmail = async (email: string, otp: string): Promise<void> => {
  ...
  if (isSmtpConfigured) {
    ...
    const mailOptions = {
      from: env.SMTP_FROM || `"FinTrack" <no-reply@fintrack.app>`,
      to: email,
      subject: 'Verify Your Email Address - FinTrack',
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 12px;">
          <h2 style="color: #111; margin-bottom: 16px;">FinTrack</h2>
...
export const sendReportEmail = async (
  ...
  if (isSmtpConfigured) {
    ...
    console.log(`[MAILER MOCK] Attached File: ${fileName}`);
    console.log('=============================================\n');
  }
};
